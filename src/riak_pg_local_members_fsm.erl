...
%% @doc Local Members FSM.

-module(riak_pg_local_members_fsm).
-author('Christopher Meiklejohn <christopher.meiklejohn@gmail.com>').
...
%% @doc Get local members.
local_members(Group) ->
    ReqId = riak_pg:mk_reqid(),
    riak_pg_local_members_fsm_sup:start_child([ReqId, self(), Group]),
    {ok, ReqId}.
...
execute(timeout, #state{preflist=Preflist,
                        ...
                        group=Group}=State) ->
    riak_pg_memberships_vnode:local_members(Preflist, {ReqId, Coordinator}, Group),
