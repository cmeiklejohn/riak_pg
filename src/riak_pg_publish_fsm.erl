...
publish(Channel, Message) ->
    ...
    riak_pg_publish_fsm_sup:start_child(
...
execute(timeout, #state{preflist=Preflist,
    ...
    riak_pg_publications_vnode:publish(Preflist,
                                           {ReqId, Coordinator},
                                           Channel, Message),
...
repair([{IndexNode, Pids}|Replies],
    ...
    case riak_dt_orset:equal(Pids, MPids) of
        false ->
            riak_pg_subscriptions_vnode:repair(IndexNode, Channel, MPids);
        true ->
            ok
    end,
